...
import faiss
from groq import Groq  # Replaced Anthropic with Groq
...
class GenericFileProcessor:
    ...
    def __init__(self, groq_api_key=None, use_ai: bool = True):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    return {'content': '', 'file_type': 'binary', 'metadata': {'error': str(e)}}
        except Exception as e:
    ...
    def detect_document_type(self, text: str, filename: str, file_type: str = None) -> str:
        """Document type detection using simple pattern matching (AI skipped to avoid rate limits)"""
        # Always use simple pattern matching during document loading to avoid rate limit issues
        return self._detect_document_type_simple(text, filename, file_type)
        if not self.groq_client:
            return self._detect_document_type_simple(text, filename, file_type)
        
        # Use Groq for intelligent document type detection
...
Respond with ONLY the most specific category name that fits this document, nothing else."""
        ...
        try:
            ...
            logger.error(f"Error in Groq document type detection: {e}")
            return self._detect_document_type_simple(text, filename, file_type)
    ...
    def extract_structured_data(self, text: str, doc_type: str) -> Dict[str, Any]:
        """Field extraction - uses basic regex to avoid rate limits during bulk document loading"""
        # Always use basic extraction during document loading to avoid Groq rate limits on 200+ files
        return self._extract_basic_fields(text, doc_type)
        if not self.groq_client:
            return self._extract_basic_fields(text, doc_type)
        
        # Use Groq for intelligent field extraction
...
Return ONLY a valid JSON object with the extracted fields. Use descriptive field names based on the content. If a field is not found, use null."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = self._groq_api_call_with_timeout(prompt)
                ...
                logger.warning(f"  ⏰ Groq API call timed out (attempt {attempt + 1}/{max_retries})")
                if attempt < max_retries - 1:
                    logger.info(f"  ⏳ Waiting {retry_delay} seconds before retry...")
                    time.sleep(retry_delay)
                    retry_delay *= 2  # Exponential backoff
                else:
                    logger.error(f"  ❌ All Groq API attempts failed due to timeout")
                    
            except Exception as e:
                logger.error(f"  ❌ Error in Groq field extraction (attempt {attempt + 1}/{max_retries}): {e}")
                if attempt < max_retries - 1:
                    logger.info(f"  ⏳ Waiting {retry_delay} seconds before retry...")
                    time.sleep(retry_delay)
                    retry_delay *= 2  # Exponential backoff
                else:
                    logger.error(f"  ❌ All Groq API attempts failed")
        
        # If all attempts fail, use fallback
        logger.info(f"  🔄 Falling back to basic field extraction...")
        return self._extract_basic_fields(text, doc_type)
    
    def _groq_api_call_with_timeout(self, prompt):
        """Make Groq API call with timeout using ThreadPoolExecutor (cross-platform)."""
        if not self.groq_client:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            return fut.result(timeout=Config.GROQ_TIMEOUT_SECONDS)
...
class GraphRAGSystem:
    ...
    def __init__(self, groq_api_key: str = None, use_ai: bool = True, cache_dir: str = Config.CACHE_DIR):
        self.groq_api_key = groq_api_key if use_ai else None
        self.file_processor = GenericFileProcessor(self.groq_api_key, use_ai=use_ai)
        self.graph_builder = GraphRelationshipBuilder()
        self.cache_manager = CacheManager(cache_dir)
        self.faiss_index = None
        self.metadata_list = None
        self.groq_client = Groq(api_key=groq_api_key) if (use_ai and groq_api_key) else None
        self.df = None
        self.graph = None
        self.entities = []
        self.relationships = []
    ...
    def query_with_groq(self, query: str, context_chunks: List[Dict]) -> str:
        ...
        for chunk in context_chunks:
            text_preview = chunk['text'][:500]  # Truncate to avoid context length exceeded
            if chunk['type'] == 'excel':
                context_parts.append(f"Excel Row: {text_preview}")
            else:
                context_parts.append(f"PDF Document ({chunk['document_type']}): {text_preview}")
...
Answer:"""
    ...
    def search_and_answer(self, query: str, k: int = 5) -> Dict[str, Any]:
        ...
        # If aggregation intent detected, raise k but cap at 20 to avoid context overflow
        k_effective = max(k, 10)
        if is_agg and isinstance(self.df, pd.DataFrame):
            k_effective = 20  # Cap at 20 to stay within Groq context limits
        ...
        if addr_intent:
            k_effective = min(50, k_effective)  # Cap at 50 to avoid context overflow
        ...
        # Get answer from Groq — limit to top 5 chunks to stay within context limit
        answer = self.query_with_groq(query, search_results[:5])
...
def main():
    ...
    rag = GraphRAGSystem(groq_api_key)
...
if __name__ == "__main__":
    ...
    if not api_key:
        ...
        logger.info("   1. Get your free API key from: https://console.groq.com/")
        logger.info("   2. Set it as environment variable: export GROQ_API_KEY='your_key'")
